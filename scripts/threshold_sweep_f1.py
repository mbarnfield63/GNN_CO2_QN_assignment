...
def main():
    ...
    for t in THRESHOLDS:
        ...
        ).mean() * 100 if n_ret > 0 else float("nan")

        ret_pct = n_ret / n_test_total * 100

        f1s = []
        for true_col, pred_col in QN_PAIRS:
            if true_col in retained.columns and pred_col in retained.columns and n_ret > 0:
                f1 = f1_score(retained[true_col], retained[pred_col],
    ...
    results["f1_drop"] = results["mean_f1"] - baseline_f1  # negative = worse than baseline
